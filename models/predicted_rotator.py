...
# TODO am I able to inherit from the Rotator class?
class PredictiveRotator(SingleClassifier):
    img_fc: nn.Module
    lang_fc: nn.Module
    cls_fc: nn.Module
    state_fc: nn.Module
    ...
    def forward(self, batch, teacher_force=True, init_view_force=None):
        ...
        init_best_views1 = init_state_estimation['best_views1']
        init_best_views2 = init_state_estimation['best_views2']
        ...
        # TODO either I use random or predicted for training..
        ...
        if self.training:
            ...
            img1_chosen_feats = torch.stack([img1_n_feats[i, [init_views1[i], rotated_views1[i]], :].max(dim=-2)[0]
                                       for i in range(bs)])
            img2_chosen_feats = torch.stack([img2_n_feats[i, [init_views2[i], rotated_views2[i]], :].max(dim=-2)[0]
                                        for i in range(bs)])
        ...
        final_state_estimation = self.estimate_state(img1_n_feats, img2_n_feats, lang_feats,
                                                     [rotated_views1, rotated_views2], self.estimate_final_state, ans)
        ...
        if not test_mode:
            ...
            # TODO: add final?!
            init_best_views = torch.tensor([init_best_views1[i] if ans[i] < 1 else init_best_views2[i] for i in range(bs)]).to(self.device)
            total_correct_best_caption_view_est = int(torch.count_nonzero(est_best_caption_view == init_best_views))
            ...
            caption_view_prediction_error = self.modulo_views(init_best_views - est_best_caption_view).abs().float().mean()
            ...
            return {
                ...
                'init_est_view_loss': init_state_estimation['est_view_loss'],
                'init_est_caption_view_loss': init_state_estimation['est_caption_view_loss'],
                'final_est_view_loss': final_state_estimation['est_view_loss'],
                'final_est_caption_view_loss': final_state_estimation['est_caption_view_loss'],
    ...
    def estimate_state(self, img1_n_feats, img2_n_feats, lang_feats, init_view_force, compute_loss, gt_img_id):
        all_probs = []
        bs = img1_n_feats.shape[0]
        ...
        if compute_loss and self.training:
            ...
            if torch.any(gt_img_id < 0):
                raise RuntimeError("cannot compute loss for caption view prediction if gt_img_id is not given!")
            best_views_per_expression = torch.tensor([best_views1[i] if gt_img_id[i] < 1 else best_views2[i] for i in range(bs)]).to(self.device)
            gt_best_view = F.one_hot(best_views_per_expression.to(torch.int64), num_classes=self.num_views)

            est_loss_weight = self.cfg['train']['loss']['est_weight']
            caption_view_est_weight = self.cfg['train']['loss']['caption_view_est_weight']
            
            view_estimation_loss = ((self.smoothed_cross_entropy(est_init_views_logits1, gt_init_views1) +
                     self.smoothed_cross_entropy(est_init_views_logits2, gt_init_views2)) / 2) * est_loss_weight
            
            caption_estimation_loss = self.smoothed_cross_entropy(est_caption_view_logits, gt_best_view) * caption_view_est_weight
            loss += view_estimation_loss + caption_estimation_loss
        else:
            view_estimation_loss = 0
            caption_estimation_loss = 0
        ...
        return {
            ...
            'loss': loss,
            'est_view_loss': view_estimation_loss,
            'est_caption_view_loss': caption_estimation_loss,
            'init_views1': init_views1,
            'init_views2': init_views2,
            'est_init_views1': est_init_views1,
            'est_init_views2': est_init_views2,
            'caption_view_est': caption_view_est,
        }
    ...
    def choose_feats_from_random_views(self, bs, img1_n_feats, img2_n_feats, init_views1, init_views2):
        ...
        # maxpooling the init and next view features
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict({
            ...
            'train/total_correct_final_view_est': out['total_correct_final_view_est'],
            'train/total_correct_best_view_est': out['total_correct_best_view_est'],
            'train/est_error': out['est_error'],
            'train/caption_view_prediction_error': out['caption_view_prediction_error'],
